...
class _PretsScreenState extends State<PretsScreen> {
  ...
  Future<void> _nouveauPret(
    ...
    // Photo de preuve : récupérée depuis PaiementChoixScreen (page suivante)
    String? photoPreuvePret;
    ...
    if (isPremium) {
      ...
      // Récupérer référence + photo depuis PaiementChoixScreen
      photoPreuvePret = paiementResult['photoPreuveBase64'];
    ...
    final ok = await afficherModalePin(
      ...
      onValider: (pin) async {
        ...
        prets.add({
          ...
          if (photoPreuvePret != null && photoPreuvePret!.isNotEmpty)
            'photo_preuve': photoPreuvePret,
        });
        newData['prets'] = prets;
    ...
    final raison = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Remboursement de ${pret.emprunteurNom} — ${Formatters.montant(remb.montant, devise: data.devise)}',
    ...
    final ok = await afficherModalePin(
      ...
      onValider: (pin) async {
        ...
        caisse.add({
          ...
          'description': 'Annulation remb. ${pret.emprunteurNom} (réf. ${remb.reference}) — Raison : $raison',
        ...
        journal.insert(0, {
          'quoi': 'ANNULATION REMBOURSEMENT — ${pret.emprunteurNom} — ${Formatters.montant(remb.montant, devise: data.devise)} — Réf: ${remb.reference} — Raison : $raison',
          'gestionnaire': provider.gestActifNom ?? '',
          'quand': now,
          'reference': ref,
        });
