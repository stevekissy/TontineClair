#!/usr/bin/env python3
"""Deploy FIX v19 via Supabase Management API avec User-Agent supabase-cli."""

import json, sys, urllib.request, urllib.error, time

TOKEN       = "sbp_v0_b7f8500e12dba3c1679de4322967f8f121c2a3d4"
...
def exec_sql(sql: str) -> tuple:
    url  = f"https://api.supabase.com/v1/projects/{PROJECT_REF}/database/query"
    body = json.dumps({"query": sql}).encode("utf-8")
    req  = urllib.request.Request(url, data=body, method="POST")
    req.add_header("Authorization",  f"Bearer {TOKEN}")
    req.add_header("Content-Type",   "application/json")
    req.add_header("User-Agent",     "supabase-cli/2.22.6")
    req.add_header("Accept",         "application/json")
    try:
        with urllib.request.urlopen(req, timeout=60) as r:
            raw = r.read().decode("utf-8")
            return r.status, json.loads(raw) if raw.strip() else [], raw
    except urllib.error.HTTPError as e:
        raw = e.read().decode("utf-8")
        try:    return e.code, json.loads(raw), raw
        except: return e.code, {}, raw

def rpc(fn, params) -> tuple:
...
BEGIN
  ...
  -- FALLBACK : JSONB voix (anciens votes)
...
step("1/5", "Section 0 : drop ancienne signature boolean...")
st, bd, raw = exec_sql(SQL_S0)
if st in (200, 201):
    ok(f"Section 0 OK (HTTP {st})")
else:
    print(f"  HTTP {st} — {str(bd)[:200]}")

time.sleep(0.4)

step("2/5", "Section 1 : proposer_nouveau_cycle (p_question → jsonb)...")
st, bd, raw = exec_sql(SQL_S1)
if st in (200, 201):
    ok(f"proposer_nouveau_cycle déployée ! (HTTP {st})")
else:
    err(f"ÉCHEC HTTP {st}")
    print(f"  {str(bd)[:400]}")
    sys.exit(1)
...
step("3/5", "Section 2 : clore_vote_redemarrage (lit table voix)...")
st, bd, raw = exec_sql(SQL_S2)
if st in (200, 201):
    ok(f"clore_vote_redemarrage déployée ! (HTTP {st})")
else:
    err(f"ÉCHEC HTTP {st}")
    print(f"  {str(bd)[:400]}")
    sys.exit(1)
...
step("4/5", "Vérification signatures en production...")
st, bd, raw = exec_sql(SQL_VERIFY)
if st in (200, 201) and isinstance(bd, list):
    all_ok = True
    for row in bd:
        fn  = row.get("fonction", "?")
        ...
        if ret == "jsonb":
            ...
            err(f"{fn}  →  {ret}  ← PROBLÈME")
            all_ok = False
    if all_ok and len(bd) == 2:
        print(f"\n{BOLD}{G}✅ Les 2 fonctions retournent jsonb — BUGs corrigés{RESET}")
    else:
        print(f"  {len(bd)} fonction(s) trouvée(s) — attendu 2")
else:
    print(f"  Vérification HTTP {st}: {str(bd)[:300]}")

time.sleep(0.3)

step("5/5", "Test RPC proposer_nouveau_cycle avec p_question...")
st2, bd2, _ = rpc("proposer_nouveau_cycle", {
    "p_code":     "TEST_INEXISTANT_XYZ123",
    "p_nom":      "test",
    "p_pin":      "0000",
    "p_question": "Test après fix v19 ?"
})
if st2 == 200:
    if isinstance(bd2, dict) and "erreur" in bd2:
        ok(f"RPC répond correctement : {bd2.get('erreur')}")
    else:
        ok(f"RPC OK : {str(bd2)[:100]}")
elif "PGRST202" in str(bd2):
    err("RPC encore introuvable — problème de déploiement")
else:
    info(f"RPC HTTP {st2} : {str(bd2)[:200]}")

print(f"\n{BOLD}{'='*54}{RESET}")
print(f"{BOLD}FIX v19 déployé — Tester sur l'app :{RESET}")
print("  Nouveau Cycle → Proposer → Membres votent → Clôturer")
print("  Résultats attendus : oui/non/abstention corrects")
